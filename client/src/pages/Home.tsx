...
import macStudioFixImage from "@/assets/nadola/mac-studio-fix.jpg";
...
const images = {
  ...
  offers: offersImage,
  macStudioFix: macStudioFixImage,
};
...
const products: Product[] = [
  ...
  { id: 13, name: "بودرة MAC Studio Fix", en: "MAC Studio Fix Powder Plus Foundation", category: "العناية بالبشرة", size: "غير محدد", image: images.macStudioFix, badge: "جديد", stock: 1, featured: true, description: "بودرة أساس MAC Studio Fix Powder Plus Foundation. السعر والتفاصيل متاحة عبر واتساب." },
];
...
function WhatsAppButton({ product, quantity = 1, className = "" }: { product?: Product; quantity?: number; className?: string }) {
  const message = product
    ? `السلام عليكم Nadola Collection 🌸%0A%0Aأرغب في طلب:%0Aالمنتج: ${product.name}%0Aالحجم: ${product.size}%0Aالكمية: ${quantity}${product.price == null ? "" : `%0Aالسعر: ${formatPrice(product.price * quantity)} SDG`}%0A%0Aأرجو تأكيد التوفر والتوصيل.`
    ...
    <article className="product-card">
      ...
      <div className="product-card__body">
        ...
        <div className="product-card__footer">
          <div>{product.price == null ? <strong className="price-on-request">السعر عبر واتساب</strong> : <><strong>{formatPrice(product.price)} <small>SDG</small></strong>{product.oldPrice ? <del>{formatPrice(product.oldPrice)} SDG</del> : null}</>}</div>
...
function Home() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (sort === "price-low") return [...list].sort((a, b) => (a.price ?? Number.MAX_SAFE_INTEGER) - (b.price ?? Number.MAX_SAFE_INTEGER));
  ...
  const addToCart = (product: Product, quantity = 1) => {
    const priceLine = product.price == null ? "" : `%0Aالسعر: ${formatPrice(product.price * quantity)} SDG`;
    window.open(`https://wa.me/${whatsappNumber}?text=السلام عليكم Nadola Collection 🌸%0A%0Aأرغب في طلب:%0Aالمنتج: ${product.name}%0Aالحجم: ${product.size}%0Aالكمية: ${quantity}${priceLine}%0A%0Aأرجو تأكيد التوفر والتوصيل.`, "_blank", "noopener,noreferrer");
  };
...
function ProductsPanel() { return <div className="admin-content"><div className="panel-toolbar"><div><p className="admin-muted">إدارة الكتالوج والمخزون والأسعار</p></div><button className="button button--dark"><Plus size={17} /> إضافة منتج</button></div><section className="admin-panel"><div className="panel-heading"><div><span className="eyebrow">CATALOG / 13 ITEMS</span><h2>كل المنتجات</h2></div><div className="toolbar-search"><Search size={15} /><input placeholder="ابحثي في المنتجات" /></div></div><div className="product-admin-grid">{products.map(product => <div className="product-admin-card" key={product.id}><img src={product.image} alt="" /><div><span className="eyebrow">{product.category}</span><h3>{product.name}</h3><p>{product.price == null ? "السعر عبر واتساب" : `${formatPrice(product.price)} SDG`} · {product.size}</p><div><span className={product.stock <= 5 ? "stock stock--low" : "stock"}>{product.stock <= 5 ? "مخزون منخفض" : "متوفر"}</span><button><Pencil size={14} /></button></div></div></div>)}</div></section></div>; }
